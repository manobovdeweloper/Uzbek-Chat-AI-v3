...
import { User, Sparkles, Copy, Check, ThumbsUp, ThumbsDown, Bookmark, ChevronDown, ChevronUp } from "lucide-react";
...
const LANG_COLORS: Record<string, string> = {
  ...
  ruby: "#f43f5e", swift: "#f97316", kotlin: "#a855f7",
};
...
function CodeBlock({ children, className }: { children?: React.ReactNode; className?: string }) {
  ...
  return (
    <div className="relative my-3 rounded-xl overflow-hidden shadow-lg" style={{ border: "1px solid hsl(220 20% 18%)" }}>
      {/* Header */}
      <div className="flex items-center justify-between px-4 py-2" style={{ background: "hsl(220 28% 10%)", borderBottom: "1px solid hsl(220 20% 18%)" }}>
...
export function MessageBubble({ message, isStreaming, isBookmarked, onBookmark, searchQuery, onCopy }: MessageBubbleProps) {
  ...
  return (
    <div className={`flex gap-3 w-full group/msg ${isUser ? "flex-row-reverse" : "flex-row"}`}
      ...
      <div className={`flex flex-col gap-1.5 max-w-[84%] min-w-0 ${isUser ? "items-end" : "items-start"}`}>
        {/* Meta */}
        <div className={`flex items-center gap-2 px-1 ${isUser ? "flex-row-reverse" : "flex-row"}`}>
          <span className="text-[11px] font-semibold text-foreground/50">{isUser ? "Siz" : "O'zbek AI"}</span>
          <button
            className="text-[10px] text-muted-foreground/38 hover:text-muted-foreground/60 transition-colors cursor-default"
        ...
        <div className={`relative rounded-2xl leading-relaxed shadow-sm text-[var(--chat-font-size,15px)] ${
          isUser
            ? "bg-gradient-to-br from-primary to-primary/85 text-white rounded-tr-sm px-4 py-3"
            : "bg-card border border-border/60 text-card-foreground rounded-tl-sm px-4 py-3.5 prose prose-sm dark:prose-invert max-w-none"
        } ${searchQuery && message.content.toLowerCase().includes(searchQuery.toLowerCase()) ? "ring-2 ring-yellow-400/40 ring-offset-1 ring-offset-background" : ""}`}>
          ...
          {isCollapsible && (
            <button onClick={() => setExpanded(!expanded)}
              ...
              {expanded
                ? <><ChevronUp className="w-3.5 h-3.5" />Yig'ish</>
                : <><ChevronDown className="w-3.5 h-3.5" />Ko'proq ko'rish ({wordCount} so'z)</>}
            </button>
          )}
        </div>
          ...
          <div className={`flex gap-1 px-1 flex-wrap ${isUser ? "justify-end" : "justify-start"}`}>
            {emojiReactions.map((e) => (
              <button key={e} onClick={() => toggleEmoji(e)}
                className="reaction-pop text-sm px-2 py-0.5 rounded-full bg-muted border border-border hover:bg-primary/10 hover:border-primary/30 transition-all leading-relaxed">
                {e}
              </button>
            ))}
          </div>
          ...
          <div className={`flex items-center gap-0.5 px-0.5 opacity-0 group-hover/msg:opacity-100 transition-opacity duration-150 ${isUser ? "flex-row-reverse" : "flex-row"}`}>
            ...
            {onBookmark && (
              <button onClick={onBookmark}
                className={`p-1.5 rounded-lg hover:bg-muted transition-all ${isBookmarked ? "text-amber-400 bg-amber-400/10" : "text-muted-foreground hover:text-amber-400"}`}
                title="Xatcho'p">
            ...
            <div className="relative">
              ...
              {showReactionPicker && (
                <div className={`absolute bottom-full mb-1.5 z-40 flex gap-1 bg-popover border border-border rounded-2xl p-2 shadow-2xl animate-in fade-in zoom-in-95 duration-100 ${isUser ? "right-0" : "left-0"}`}>
                  {REACTIONS.map((e) => (
                    <button key={e} onClick={() => toggleEmoji(e)}
                      className={`w-7 h-7 flex items-center justify-center rounded-xl text-base hover:bg-muted transition-all hover:scale-115 active:scale-95 ${emojiReactions.includes(e) ? "bg-primary/15 ring-1 ring-primary/30" : ""}`}>
                      {e}
                    </button>
                  ))}
                </div>
            ...
            {!isUser && (
              <>
                <button onClick={() => setReaction(reaction === "up" ? null : "up")}
                  className={`p-1.5 rounded-lg hover:bg-muted transition-all ${reaction === "up" ? "text-emerald-500 bg-emerald-500/10" : "text-muted-foreground hover:text-emerald-500"}`}
                  title="Yaxshi javob">
                  <ThumbsUp className="w-3.5 h-3.5" />
                </button>
                <button onClick={() => setReaction(reaction === "down" ? null : "down")}
                  className={`p-1.5 rounded-lg hover:bg-muted transition-all ${reaction === "down" ? "text-rose-500 bg-rose-500/10" : "text-muted-foreground hover:text-rose-500"}`}
                  title="Yomon javob">
